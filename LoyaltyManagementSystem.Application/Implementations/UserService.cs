...
using LoyaltyManagementSystem.Application.ViewModels;
...
namespace LoyaltyManagementSystem.Application.Implementations
{
    public class UserService : IUserService
    {
        ...
        public PageResult<AppUserViewModel> GetAllPaging(string keyword, int index, int pageSize)
        {
            var query = _userManager.Users;
            if (!string.IsNullOrEmpty(keyword))
            {
                query.Where(x => x.FullName.Contains(keyword) || x.UserName.Contains(keyword) || x.Email.Contains(keyword));
            }
